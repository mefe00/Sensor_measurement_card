...
* Kudos: https://code.google.com/p/efficient-java-matrix-library
*/
// Burası bellek optimizasyonu için çok ÖNEMLİ!!!!!!!!!!!!!!
// Burada matrix çarpımında, özellikle büyük matrislerde, cache performansını artırmak için bazı optimizasyonlar yapılabilir. Örneğin, matris B'nin sütunlarını tek tek kopyalayarak (baux kullanarak) işlemi gerçekleştirmek, cache miss'lerini azaltabilir. Ayrıca, döngü sıralamasını ve erişim desenlerini optimize etmek de performansı artırabilir. Bu tür optimizasyonlar, özellikle gömülü sistemlerde önemli olabilir, çünkü bellek erişimi genellikle işlemci hızından daha yavaştır.
