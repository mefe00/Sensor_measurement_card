...
*/
// MCU'nun donanımsal FPU'su (Floating Point Unit) varsa, matrix_data_t'yi float olarak tanımlayarak kayan nokta işlemlerini hızlandırabiliriz. Eğer FPU yoksa, matrix_data_t'yi double yaparak daha yüksek doğruluk elde edebiliriz, ancak bu durumda işlemler daha yavaş olacaktır. Projenin gereksinimlerine ve hedef platformun özelliklerine göre bu tür bir yapılandırma yapılabilir.
typedef float matrix_data_t;

/**
* \brief Matrix definition
*/
// Bu struct yapısının asıl amacı MCU'nun hantallaşmaısını engellemek içindir. Bunun için bütün matrisi tek ve uzun bir şerit halinde diziyor.
typedef struct {
    /**
